...
version = "2.0.1 - BETA"
project_title = "592-Cam | 592 Branding"
...
class CameraObject:
    ...
    def load_camera_profile(self, profile_filename):
        ...
        try:
            ...
            # âœ… Load the profile before applying any settings
            self.camera_profile = profile_data
            # âœ… Apply settings after loading the profile
            ...
            # âœ… Update camera-last-config.json
            try:
    ...
    def update_settings(self, setting_id, setting_value):
        ...
        if not updated:
            print(f"âš ï¸ Warning: Setting {setting_id} not found in live_controls!")
        ...
        if "controls" in self.camera_profile:
            try:
                for setting_id, setting_value in self.camera_profile["controls"].items():
                    self.picam2.set_controls({setting_id: setting_value})
                    self.update_settings(setting_id, setting_value)  # âœ… Use the loop variables
                    print(f"Applied Control: {setting_id} -> {setting_value}")
                print("âœ… All profile controls applied successfully")
            except Exception as e:
                print(f"âš ï¸ Error applying profile controls: {e}")
    ...
    def set_sensor_mode(self, mode_index):
        try:
            ...
            if width * height > 2304 * 1296:  # If resolution is higher than 1080p
                print(f"âš ï¸ High resolution mode detected: {width}x{height}")
            ...
            try:
                ...
                print(f"âš ï¸ Error configuring sensor mode: {e}")
            ...
            if not self.camera_init:
                try:
                    ...
                    print(f"âš ï¸ Error restarting camera: {e}")
                    self.use_placeholder = True
                    raise
                
            print(f"âœ… Sensor mode {mode_index} applied")
            ...
            if not self.camera_init:
                try:
                    ...
                    print(f"âš ï¸ Recovery failed: {recovery_error}")
                    self.use_placeholder = True
    ...
    def generate_camera_resolutions(self):
        ...
        if not self.sensor_modes:
            print("âš ï¸ Warning: No sensor modes available!")
            return []
        ...
        if not resolutions:
            print("âš ï¸ Warning: No valid resolutions found in sensor modes!")
            return []
            ...
            try:
                if self.use_placeholder:
                    ...
                    with self.output.condition:
                        ...
                        if not notified:
                            print("âš ï¸ Timed out waiting for frame.")
                            consecutive_timeouts += 1
                            if consecutive_timeouts >= max_timeouts:
                                print("âš ï¸ Too many consecutive timeouts, attempting stream recovery...")
                ...
                if frame is None or not isinstance(frame, bytes):
                    print(f"âš ï¸ Invalid frame ({type(frame)}), using placeholder.")
                    frame = self.placeholder_frame
                    continue
                ...
                try:
                    ...
                    print(f"âš ï¸ Error getting camera config: {e}")
                    frame = self.placeholder_frame
                    continue
                ...
                #        print(f"âš ï¸ Resolution mismatch: {actual_res} â‰  {expected_res}")
                #        self.safe_restart_stream()
                #        continue
                #except Exception as e:
                #    print(f"âš ï¸ Error checking resolution: {e}")
                #    frame = self.placeholder_frame
                #    continue
                ...
                print(f"ðŸš¨ Stream loop error: {e}")
                traceback.print_exc()
    ...
    def safe_restart_stream(self):
        try:
            print("ðŸ”„ Restarting stream with correct config...")
            self.use_placeholder = True
            self.stop_streaming()
            self.picam2.stop()
            time.sleep(0.2)
            ...
            self.flush_frames()
            self.use_placeholder = False
            print("âœ… Stream restarted and flushed.")
        except Exception as e:
            print(f"ðŸš¨ Failed to restart stream: {e}")
            traceback.print_exc()
...
def capture_still(camera_num):
    ...
    try:
        print(f"ðŸ“¸ Received capture request for camera {camera_num}")

        camera = cameras.get(camera_num)
        if not camera:
            print(f"âŒ Camera {camera_num} not found.")
        ...
        #   print(f"âš ï¸ Capture request too fast for camera {camera_num}. Ignoring request.")
        ...
        print(f"ðŸ“ New image filename: {image_filename}")
        ...
        if image_path:
            print(f"âœ… Image captured successfully: {image_filename}")
            return jsonify(success=True, message="Image captured successfully", image=image_filename)
        else:
            print(f"âŒ Failed to capture image for camera {camera_num}")
            return jsonify(success=False, message="Failed to capture image")

    except Exception as e:
        print(f"ðŸ”¥ Error capturing still image: {e}")
...
def update_setting():
    try:
        ...
        # âœ… At this stage, we're just verifying the data. No changes to the camera yet.
        return jsonify({
            "success": True,
